...
const SearchDay = ({ formData, handleInputChange, handleSubmit, groups, devices, loading, getDevices, columns, showMap, setShowMap }) => {
  const [validated, setValidated] = useState(false);
  const [showDateInputs, setShowDateInputs] = useState(false); // State to manage button text
  const [buttonText, setButtonText] = useState('FETCH REPORT');
      ...
      <CCol xs={12}>
        <div className="d-flex justify-content-end">
          <div className="btn-group">
            <button className="btn btn-primary" type="submit" onClick={() => handleDropdownClick('FETCH REPORT')}>
              {buttonText}
            </button>
            <button
              type="button"
            ...
            {isDropdownOpen && (
              <ul className="dropdown-menu show">
                <li>
                  <a className="dropdown-item" href="#" onClick={() => handleDropdownClick('FETCH REPORT')}>
                    FETCH REPORT
                  </a>
                </li>
                <li>
...
const ShowDay = ({ apiData, selectedColumns, allDates, devices }) => {
        ...
        <CRow className="justify-content-center mt-4">
          <CCol xs={12} className="px-4">
            <CCard className="p-0 mb-4 shadow-sm">
              <CCardHeader className="d-flex justify-content-between align-items-center bg-secondary text-white">
                <strong>All Day Report List {selectedDeviceName && `for ${selectedDeviceName}`}</strong> {/* Show the device name here */}
                <CFormInput
                  placeholder="Search..."
                  value={searchQuery}
              ...
              <CCardBody>
                <ShowDay apiData={apiData} allDates={allDates} devices={devices} selectedColumns={selectedColumns} />
              </CCardBody>
            </CCard>
          </CCol>
        </CRow>
